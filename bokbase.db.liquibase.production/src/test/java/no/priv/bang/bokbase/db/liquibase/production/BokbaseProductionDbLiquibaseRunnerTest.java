...
class BokbaseProductionDbLiquibaseRunnerTest {
    ...
    void testCreateAndVerifySomeDataInSomeTables() throws Exception {
        ...
        BokbaseProductionDbLiquibaseRunner runner = new BokbaseProductionDbLiquibaseRunner();
    ...
    private int addAccount(DataSource datasource, String username) throws Exception {
        try (Connection connection = datasource.getConnection()) {
            try(PreparedStatement statement = connection.prepareStatement("insert into bokbase_accounts (username) values (?)")) {
        ...
        int accountId = -1;
        try (Connection connection = datasource.getConnection()) {
            try(PreparedStatement statement = connection.prepareStatement("select * from bokbase_accounts where username=?")) {
                statement.setString(1, username);
                try (ResultSet results = statement.executeQuery()) {
                    results.next();
