...
class BokbaseTestdataTest {

    @Test
    void testActivate() {
        BokbaseTestdata testdata = new BokbaseTestdata();
        ...
        when(useradmin.getRoles()).thenReturn(Arrays.asList(Role.with().build(), Role.with().build(), Role.with().id(2).rolename(BOKBASEUSER_ROLE).build()));
